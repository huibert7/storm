package org.storm.util.encryption;
...
public class SHA1
{
	...
	{
		int nI;
		for (nI = 0; nI < m_state.length; nI++) m_state[nI] = 0;
		m_lCount = 0;
		for (nI = 0; nI < m_digestBits.length; nI++) m_digestBits[nI] = 0;
		for (nI = 0; nI < m_block.length; nI++) m_block[nI] = 0;
		m_nBlockIndex = 0;
	}
	...
	{
		return (m_block[nI & 15] = rol(m_block[(nI +  13) & 15] ^ m_block[(nI + 8) & 15] ^ m_block[(nI + 2) & 15] ^ m_block[nI & 15], 1));
	}
	...
	{
		for (int nI = 0; nI < data.length; nI++) update(data[nI]);
	}
	...
	public void finalize()
	{
		...
		update((byte) 128);
		while (m_nBlockIndex != 56) update((byte) 0);

		for (nI = 0; nI < bits.length; nI++) update(bits[nI]);
	...
	public boolean selfTest()
	{
		...
		tester.clear();
		for (int nI = 0; nI < DIGEST_SIZE; nI++)
        if (digest[nI] != SELFTEST_DIGEST[nI]) return false;
